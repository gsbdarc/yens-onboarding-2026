...
Four Slurm scripts mapping 1:1 to the diagrams in the
["Ways to Parallelize"](../../docs/reference/parallelization.md) section.
Each extracts SEC filings from `data/aws_links.csv` — the same source as the
Day 2 batch job — varying only *how* the work is spread.
...
Expect well under a minute for the serial baseline (Day 2 measured ~2.25s per
