---
layout: default
title: "Part 2 — Submit a Job Array"
parent: "Day 2 — The Yen-Slurm Cluster"
nav_order: 2
has_children: true
has_toc: false
permalink: /day2/part2/
---

# Part 2 — Submit a Job Array

---

## Sections

| Section | Format | What you'll learn |
|---|---|---|
| [1. Hello World Array]({{ '/day2/hello-world-array/' | relative_url }}) | 💻 Hands-on | One submission, four tasks, four logs — the shortest proof an array works |
| [2. Estimate 100 Filings Resources]({{ '/day2/estimate-100-filings/' | relative_url }}) | 💻 Hands-on | Predict what one task needs, build the array, then check yourself against `sacct` |
| [3. Make Your Tasks Rerun-Safe]({{ '/day2/rerun-safe-tasks/' | relative_url }}) | 💻 Hands-on | Make a task skip work it has already done, so a resubmit costs nothing |
| [4. Scale]({{ '/day2/scale/' | relative_url }}) | 💻 Hands-on | Run all ~992 filings, find out why it is not one number in one directive, and document it |
| [Part 2 Checkpoint]({{ '/day2/part2-checkpoint/' | relative_url }}) | ✅ Checkpoint | Fan a job array out, make it safe to rerun, and size a bigger run before submitting it |
