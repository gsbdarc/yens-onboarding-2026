...
**Optional practice — Debug `fix_me.slurm`**
...
{: .note }
> 💡 These error logs come back later — a later optional practice uses a failed job's `logs/fix_me_*.err`. Even if you don't fix all of them, **submit at least the first one and let it fail** so you have a `logs/fix_me_*.err` to use then.
...
**Optional practice — Debug `fix_me_2.slurm`**
...
**Optional practice — Debug `fix_me_3.slurm`**
...
**Optional practice — Debug `extract_form_3_one_file_broken.slurm`**
...
**Optional practice — The `dev` partition**
