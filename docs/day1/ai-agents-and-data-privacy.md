...
{: .important }
> **In this section:** Understand how AI coding agents work at Stanford: where LLMs currently stand, how researchers are actually using them, and what happens to your data when you do.
...
Look back at the diagram: *you pick every piece.* That was true in [Extracting Data with an LLM]({{ '/day1/extracting-data-with-an-llm/' | relative_url }}), where you wrote both prompts yourself and knew exactly what the model received. A coding agent fills that window for you, with:
...
**Job 1: an LLM analyzes your data.** Your data *is* the input, the way you fed filing text to the model in [Extracting Data with an LLM]({{ '/day1/extracting-data-with-an-llm/' | relative_url }}). One question decides everything: *is this data allowed to go where this model runs?*
...
You met the headline in [The Stanford AI Playground]({{ '/day1/stanford-ai-services/' | relative_url }}): the chat window is free, the API Gateway is metered. Here is where the meter actually runs.
...
|---|---|
| **How much you send** | The `[:4000]` slice in Extracting Data with an LLM was a cost decision as much as a context-limit one |
...
Nothing is broken here; that's how these models work. The trap is reading cost off the length of the reply, so read `usage` instead. If you want to watch it happen on a live call, the *Pay for Thinking You Never See* optional practice in [Extracting Data with an LLM]({{ '/day1/extracting-data-with-an-llm/' | relative_url }}) runs exactly this comparison.
